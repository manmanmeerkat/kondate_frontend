...
import { useWesternFukusai } from "../../../hooks/useWesternFukusai";


interface AllMyDishesProps {}

export const WesternFukusai: React.FC<AllMyDishesProps> = memo(() => {
  const { isOpen, onOpen, onClose } = useDisclosure();
  const { dishes, loading } = useAllMyDishes();
  const { WesternFukusai } = useWesternFukusai()
          ...
          {noSearchResults ? (
            ...
            <>
              {searchedRecipes.length > 0 ? (
                ...
                WesternFukusai.length > 0 ? (
                  WesternFukusai.map((dish: any) => (
                    <WrapItem key={dish.id} mx="auto">
                      <DishCard
                        id={dish.id}
                        imageUrl={dish.image_path}
