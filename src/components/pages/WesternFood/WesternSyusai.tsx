...
export const WesternSyusai: React.FC<JapaneseProps> = memo(() => {
  ...
  const { getWesternSyusai, dishes, loading } = useAllMyDishes();
  const { onSelectDish, selectedDish } = useSelectDish();
  const { searchedRecipes, handleIngredientSearch } = useIngredientSearch("western-syusai");
  ...
  return (
    <div>
      <Header />
      <GenreButton />
      <InputGroup mt={4} mx="auto" w={{ base: "30%", md: "30%" }}>
        <Input
          placeholder="材料から検索"
          value={searchKeyword}
