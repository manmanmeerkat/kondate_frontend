...
export const WesternSyusai: React.FC<AllMyDishesProps> = memo(() => {
  ...
  const { searchedRecipes, handleIngredientSearch } = useIngredientSearch("western-syusai",user?.id);
          ...
          {noSearchResults ? (
            ...
            <>
              {searchedRecipes.length > 0 ? (
                ...
                WesternSyusai.length > 0 ? (
                  WesternSyusai.map((dish: any) => (
                    <WrapItem key={dish.id} mx="auto">
                      <DishCard
                        id={dish.id}
                        imageUrl={dish.image_path}
