...
export const OthersOthers: React.FC<OtheresProps> = memo(() => {
  const { isOpen, onOpen, onClose } = useDisclosure(); // モーダルの開閉状態を管理するためのフック
  const { getOthersOthers, dishes, loading } = useAllMyDishes(); // 他のカテゴリの料理データの取得とローディング状態を管理
  const { data } = useOthersOthers(); // カスタムフックを使用してデータを取得
  const { onSelectDish, selectedDish } = useSelectDish(); // 選択された料理を管理
  const { user } = useFetchUserData(); // ユーザー情報を取得するカスタムフック
  const { searchedDishes, handleIngredientSearch } = useIngredientSearch("others-others", user?.id); // 材料検索用のカスタムフック
                ...
                {selectedDishes.map((dish: Dish) => (
                  <WrapItem key={dish.id} mx="auto">
                    <DishCard
                      ...
                      menuType="OthersOthers" // メニュータイプを指定
                      dishName={dish.name}
