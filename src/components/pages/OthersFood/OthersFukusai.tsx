...
export const OthersFukusai: React.FC<OthersProps> = memo(() => {
  ...
  const { getOthersFukusai, dishes, loading } = useAllMyDishes(); // その他の副菜データの取得とローディング状態を管理
  const { data } = useOthersFukusai(); // 副菜のデータを取得するカスタムフック
  const { onSelectDish, selectedDish } = useSelectDish(); // 選択された料理を管理
  const { user } = useFetchUserData(); // ユーザー情報を取得するカスタムフック
  const { handleIngredientSearch } = useIngredientSearch("others-fukusai", user?.id); // 材料検索用のカスタムフック
                ...
                {selectedDishes.map((dish: Dish) => (
                  <WrapItem key={dish.id} mx="auto">
                    <DishCard
                      ...
                      menuType="OthersFukusai" // メニュータイプを指定
                      dishName={dish.name}
